...
<body>
    <header class="header">
        <div class="header__inner">
            <a href="{{ route('items.showIndex') }}" class="header__inner-logo">
            ...
            <div class="trade__messages">
                ...
                <div class="message__block {{ $message->user_id === auth()->id() ? 'mine' : '' }}">
                    ...
                    <div class="message {{ $message->user_id === auth()->id() ? 'mine' : 'other' }}">
                        @if ($message->content)
                        {{-- 通常テキスト表示（編集中は非表示になる） --}}
                        <p class="message__text" id="text-{{ $message->id }}">{{ $message->content }}</p>
                        @endif
                        ...
                        <form id="edit-form-{{ $message->id }}"
                            ...
                            @csrf
                            @method('PATCH')
                            <input type="text" name="content" value="{{ $message->content }}" maxlength="400" class="message__edit-input">
                            <button type="submit" class="message__update-btn">編集</button>
                        </form>
                    </div>
                    ...
                    <div class="message__actions">
                        <button type="button" class="message__actions-btn edit-btn"
                            data-id="{{ $message->id }}"
